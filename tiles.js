...
class Tiles {
    ...
    if (direction === "R") {
      ...
      while (leftovers.length > 0) {
        ...
        row.forEach((tt) => {
          ...
          this.tiles[tt.row][tt.col] = this.createEmptyTile({
            row: tt.row,
            col: tt.col,
  ...
  detectBorder(tiles, direction) {
    ...
    if (direction === "R") {
      ...
      while (leftovers.length > 0) {
        ...
        const rightmostCol = Math.max.apply(
          null,
          tilesInThisRow.map((tt) => tt.col)
        );
    ...
    if (direction === "D") {
      ...
      while (leftovers.length > 0) {
        ...
        const downmostRow = Math.max.apply(
          null,
          tilesInThisCol.map((tt) => tt.row)
        );
  ...
  detectMoveConflicts(tiles, direction) {
    ...
    for (let i = 0; i < border.length; i++) {
      ...
      if (
        neighborRow >= this.dim.rows ||
        neighborRow < 0 ||
        neighborCol >= this.dim.cols ||
        neighborCol < 0
      ) {
        return false;
      }
  ...
  expandGrid(direction, amount) {
    ...
    if (direction === "D") {
      ...
      this.expandGrid("R");
      this.moveTiles(
        this.asFlatArray().filter((t) => t.p !== "E"),
        "R"
      ); // move all non empty tiles left
    }
  }
}
