(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function toString(obj) {
    ...
    try {
      ...
      if (obj == null) {
        ...
        (obj != null ? obj.toString : void 0) != null &&
  ...
  function processAfters() {
    ...
    try {
      while (aftersStackHeight >= stackSizeAtStart) {
        // to prevent sinking to levels started by others
        ...
        if (index < topAfters.length) {
          ...
          try {
            ...
            callSuccess = true;
            while (
              aftersStackHeight > stackSizeAtStart &&
              aftersStack[aftersStackHeight - 1][0].length == 0
            ) {
              aftersStackHeight--;
            }
          } finally {
  ...
  var Desc = /** @class */ (function () {
    ...
    Desc.prototype.toString = function () {
      ...
      return (
        _.toString(this.context) +
        "." +
        _.toString(this.method) +
        "(" +
        args +
        ")"
      );
  ...
  function findDeps(x) {
    if (isArray(x)) {
      ...
      typeof x !== "undefined" && x !== null ? x._isSource : undefined
    ) {
      return [x.obs];
    } else {
      return [];
  ...
  function scan(src, seed, f) {
    ...
    return (resultProperty = new Property(
      new Desc(src, "scan", [seed, f]),
      subscribe
    ));
  }
  ...
  function propertyFromStreamSubscribe(desc, subscribe) {
    ...
    return new Property(
      desc,
      streamSubscribeToPropertySubscribe(none(), subscribe)
    );
  }
   ...
   */
  function once(value) {
    var s = new EventStream(new Desc("Bacon", "once", [value]), function (
      sink
    ) {
  ...
  function when_(ctor, patterns) {
    ...
    var _a = processRawPatterns(extractRawPatterns(patterns)),
      sources = _a[0],
      ixPats = _a[1];
    if (!sources.length) {
      return never();
              ...
              for (var i = 0, p; i < ixPats.length; i++) {
                ...
                if (match(p)) {
                  ...
                  for (var j = 0; j < p.ixs.length; j++) {
                    ...
                    if (!event_1) throw new Error("Event was undefined");
  ...
  function processRawPatterns(rawPatterns) {
    ...
    for (var i = 0; i < rawPatterns.length; i++) {
      ...
      if (patSources.length > 0 && !triggerFound) {
        throw new Error(
          "At least one EventStream required, none found in " + patSources
        );
      }
  ...
  function isTypedOrRawPattern(pattern) {
    return (
      pattern instanceof Array && !isObservable(pattern[pattern.length - 1])
    );
  }
  ...
  function combineWith() {
    ...
    // TODO: untyped
    var _a = argumentsToObservablesAndFunction(arguments),
      streams = _a[0],
      f = _a[1];
    var desc = new Desc("Bacon", "combineWith", __spreadArrays([f], streams));
    return combineAsArray(streams)
      .map(function (values) {
  ...
  function combineTwo(left, right, f) {
    return whenP([[wrap(left), wrap(right)], f]).withDesc(
      new Desc(left, "combine", [right, f])
    );
  }
  function wrap(obs) {
  ...
  function fromBinder(binder, eventTransformer) {
    ...
    return new EventStream(desc, function (sink) {
      ...
      var unbinder = binder(function () {
        ...
        var valueArray =
          isArray(value_) && isEvent(_.last(value_)) ? value_ : [value_];
        var reply = more;
  ...
  function sampledByE(samplee, sampler, f) {
    return sampledByP(samplee.toProperty(), sampler, f).withDesc(
      new Desc(samplee, "sampledBy", [sampler])
    );
  }
  /** @hidden */
  ...
  function transformP(src, transformer, desc) {
    return new Property(new Desc(src, "transform", [transformer]), function (
      sink
    ) {
  ...
  function withPredicate(src, f, predicateTransformer, desc) {
    if (f instanceof Property) {
      return withLatestFrom(src, f, function (p, v) {
        ...
        composeT(
          predicateTransformer(function (tuple) {
            return tuple[1];
          }),
          mapT(function (tuple) {
            return tuple[0];
  ...
  function and(left, right) {
    return left
      .combine(toProperty(right), function (x, y) {
        return !!(x && y);
      })
  ...
  function concatAll() {
    ...
    return (
      streams.length
        ? fold(tail(streams), head(streams).toEventStream(), function (a, b) {
            return a.concat(b);
          })
        : never()
  ...
  function bufferWithTime(src, delay) {
    return bufferWithTimeOrCount(src, delay, Number.MAX_VALUE).withDesc(
      new Desc(src, "bufferWithTime", [delay])
    );
  }
  /** @hidden */
  function bufferWithCount(src, count) {
    return bufferWithTimeOrCount(src, undefined, count).withDesc(
      new Desc(src, "bufferWithCount", [count])
    );
  }
  /** @hidden */
  ...
  function mergeAll() {
    ...
    if (flattenedStreams.length) {
      return new EventStream(
        new Desc("Bacon", "mergeAll", flattenedStreams),
        function (sink) {
          var ends = 0;
  ...
  function delay(src, delay) {
    return src.transformChanges(
      new Desc(src, "delay", [delay]),
      function (changes) {
  ...
  function debounceImmediate(src, delay) {
    return src.transformChanges(
      new Desc(src, "debounceImmediate", [delay]),
      function (changes) {
    ...
    return src.transform(
      composeT(
        ...
        mapT(function (firstValue) {
          ...
          var limited = limitF(data, firstValue)
            .toEventStream()
            .transform(function (event, sink) {
              var reply = sink(event);
              if (event.isEnd) {
  ...
  function diff(src, start, f) {
    return transformP(
      scan(src, [start, nullMarker], function (prevTuple, next) {
        return [next, f(prevTuple[0], next)];
      }),
      composeT(
        filterT(function (tuple) {
  ...
  function flatScan(src, seed, f) {
    ...
    return src
      ...
      .toProperty()
      .startWith(seed)
      .withDesc(new Desc(src, "flatScan", [seed, f]));
  }
  ...
  function zipWith(f) {
    ...
    return when([streams, f]).withDesc(
      new Desc("Bacon", "zipWith", [f].concat(streams))
    );
  }
  /** @hidden */
  function zip(left, right, f) {
    return zipWith(f || Array, left, right).withDesc(
      new Desc(left, "zip", [right])
    );
  }
  ...
  function combineTemplate(template) {
    ...
    var resultProperty = containsObservables(template)
      ...
      : constant(template);
    return resultProperty.withDesc(
      new Desc("Bacon", "combineTemplate", [template])
    );
  }
  ...
  function decode(src, cases) {
    return src
      .combine(combineTemplate(cases), function (key, values) {
        return values[key];
      })
  ...
  properties with array value. The result is a [Property](property.html).
    ...
    Observable.prototype.combine = function (right, f) {
      return combineTwo(this, right, f).withDesc(
        new Desc(this, "combine", [right, f])
      );
    };
    /**
  ...
  *Note:* `neverEndingStream.last()` creates the stream which doesn't produce any events and never ends.
  ...
  [1,2] - [2,3] - [3,4] - [4,5].
    ...
    Observable.prototype.subscribe = function (sink) {
      ...
      return UpdateBarrier.wrappedSubscribe(
        this,
        function (sink) {
  ...
  */
    Observable.prototype.withDescription = function (context, method) {
      ...
      this.desc = describe.apply(
        void 0,
        __spreadArrays([context, method], args)
      );
      return this;
    };
    /**
  ...
  var Property = /** @class */ (function (_super) {
    ...
    Property.prototype.concat = function (other) {
      return this.transformChanges(
        describe(this, "concat", other),
        function (changes) {
   ...
   */
    ...
    EventStream.prototype.toProperty = function (initValue) {
      ...
      return new Property(
        desc,
        streamSubscribeToPropertySubscribe(usedInitValue, streamSubscribe)
      );
    };
  ...
  var ESObservable = /** @class */ (function () {
    ...
    ESObservable.prototype.subscribe = function (
      observerOrOnNext,
      onError,
      onComplete
    ) {
  ...
  Observable.prototype[symbol("observable")] =
    Observable.prototype.toESObservable;
  ...
  function update(initial) {
    ...
    return when
      .apply(void 0, rawPatterns)
      .scan(initial, function (x, f) {
        return f(x);
      })
      .withDesc(
        new Desc("Bacon", "update", __spreadArrays([initial], patterns))
      );
  }
  ...
  function fromArray(values) {
    ...
    if (!values.length) {
      ...
      var stream = new EventStream(
        new Desc("Bacon", "fromArray", [values]),
        function (sink) {
          var unsubd = false;
          var reply = more;
          var pushing = false;
  ...
  function fromEvent(target, eventSource, eventTransformer) {
    var _a = findHandlerMethods(target),
      sub = _a[0],
      unsub = _a[1];
  ...
  function onValues() {
    ...
    return combineAsArray(args.slice(0, args.length - 1)).onValues(
        ...
        return source(count)
          ...
          .transform(function (event, sink) {
            if (isError(event)) {
              ...
              if (
                !(
                  isRetryable(errorEvent.error) &&
                  (retries === 0 || retriesDone < retries)
                )
              ) {
                finished = true;
                return sink(event);
  ...
  function fromNodeCallback(f) {
    ...
    return fromBinder(
      ...
      new Desc("Bacon", "fromNodeCallback", __spreadArrays([f], args))
    );
  }
  ...
  var Bus = /** @class */ (function (_super) {
    ...
    function Bus() {
      var _this =
        _super.call(this, new Desc("Bacon", "Bus", []), function (sink) {
    ...
    Bus.prototype.subscribeInput = function (subscription) {
      subscription.unsub = subscription.input.subscribeInternal(
        this.guardedSink(subscription.input)
      );
  ...
  var $ = {
    ...
    asEventStream: function (eventName, selector, eventTransformer) {
      ...
      }, eventTransformer).withDesc(
        new Desc(this.selector || this, "asEventStream", [eventName])
      );
    },
    /**
