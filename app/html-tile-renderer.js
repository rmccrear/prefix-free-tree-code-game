...
export function repaint(board, opts) {
  ...
  if (
    !lastTilePosition ||
    lastTilePosition.length !== tiles.length ||
    lastTilePosition[0].length !== tiles[0].length
  ) {
    for (let i = 0; i < tiles.length; i++) {
      for (let j = 0; j < tiles[i].length; j++) {
        ...
        const draggable =
          t.p !== "A"
            ? ""
            : ` draggable=true data-row=${i} data-col=${j} data-n=${t.n} data-l="${t.l}" `;
        const tileContainer = $(
          `<div class="pipe-container" style="${styleStr}" data-row="${i}" data-col="${j}"></div>`
        );
        // const tileDiv = $(`<div class="pipe-tile ${classes(t)}" ${draggable}><div class="pipe-letter">${letter}</div></div>`);
        const tileDiv = $(
          `<div class="pipe-tile ${classes(
            t
          )}" ${draggable}><p class="pipe-letter">${letter}</p></div>`
        );
    ...
    for (let i = 0; i < tiles.length; i++) {
      for (let j = 0; j < tiles[i].length; j++) {
        ...
        // Set letter
        if (diff.letter !== null) {
          // need to check for null because '' is a valid diff value
          $(`[data-row=${i}][data-col=${j}] .pipe-letter`).text(diff.letter);
        }
        // set classes
        if (diff.classesToAdd || diff.classesToRemove) {
          // no need to check for null, because '' means no change/no diff
          $(`[data-row=${i}][data-col=${j}] .pipe-tile`)
            .addClass(diff.classesToAdd)
            .removeClass(diff.classesToRemove);
        }
        // remove Alpha
        if (lastTile.p === "A" && tile.p !== "A") {
          //remove draggable
          $(`[data-row=${i}][data-col=${j}] .pipe-tile`).removeAttr(
            "draggable"
          );
          $(`[data-row=${i}][data-col=${j}] .pipe-tile`)
            .removeData("row")
        ...
        if (tile.p === "A") {
          $(`[data-row=${i}][data-col=${j}] .pipe-tile`).attr(
            "draggable",
            true
          );
          $(`[data-row=${i}][data-col=${j}] .pipe-tile`)
  ...
  $(".pipe-tile.pipe-A").on("drop", function (e) {
    ...
    if (handleSwap) {
      handleSwap(
        { row: dropped.row, col: dropped.col, l: dropped.l },
        {
          row: target.data("row"),
          col: target.data("col"),
          l: target.data("l"),
